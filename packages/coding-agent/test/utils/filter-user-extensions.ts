...
//    or `../`-relative entry pointing somewhere else under `~/.gjc/`), plus
//    the legacy non-XDG `~/.gjc/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.gjc/agent` or `$PI_CODING_AGENT_DIR`)
...
//    Handles installed plugin extensions that live outside `~/.gjc` when
...
// symlinks `~/.gjc/agent/extensions/foo.ts` to a dotfiles repo would see the
