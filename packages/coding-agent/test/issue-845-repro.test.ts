...
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the gjc path that
...
// does not follow filesystem links, so it misclassified Bun-installed gjc
// as "binary" and tried to swap gjc.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	beforeAll(() => {
		...
		fs.writeFileSync(path.join(realBinDir, "gjc"), "#!/bin/sh\n", { mode: 0o755 });

		linkedBinDir = path.join(tmpRoot, "link-bin");
		fs.symlinkSync(realBinDir, linkedBinDir, "dir");
		ompPathViaLink = path.join(linkedBinDir, "gjc");
	});
	...
	it("classifies gjc reached through a symlinked bin dir as bun-managed", () => {
	...
	it("classifies gjc at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const ompAtReal = path.join(realBinDir, "gjc");
