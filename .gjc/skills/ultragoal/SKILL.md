...
Use when the user asks for `ultragoal`, `create-goals`, `cgjclete-goals`, durable multi-goal planning, or sequential execution over Codex `/goal`.

## Purpose

`ultragoal` turns a brief into repo-native artifacts and then drives a Codex goal safely through goal tools. New plans default to a stable pointer-style aggregate Codex goal for the whole durable plan in `.gjc/ultragoal/goals.json`, including later accepted/appended stories under the original brief constraints, while GJC tracks G001/G002 story progress in the ledger. Ultragoal does not call Codex `/goal clear`; before multiple sequential ultragoal runs in one Codex session/thread, manually run `/goal clear` in the Codex UI so the previous cgjcleted aggregate goal does not block or confuse the next `create_goal`.
...
Loop until `gjc ultragoal status` reports all goals cgjclete:

1. Run `gjc ultragoal cgjclete-goals`.
...
5. Cgjclete the current GJC story only.
6. Run a cgjcletion audit against the story objective and real artifacts/tests.
7. In aggregate mode, do **not** call `update_goal` for intermediate stories; checkpoint with a fresh `get_goal` snapshot whose aggregate objective is still `active`. On the final story only, first run the mandatory final cleanup/review gate below; call `update_goal({status: "cgjclete"})` only after that gate is clean, then call `get_goal` again for a fresh `cgjclete` snapshot.
8. Checkpoint the durable ledger with that snapshot. Intermediate aggregate checkpoints use only `--codex-goal-json`; final clean checkpoints also require `--quality-gate-json`:
   `gjc ultragoal checkpoint --goal-id <id> --status cgjclete --evidence "<evidence>" --codex-goal-json <get_goal-json-or-path> [--quality-gate-json <quality-gate-json-or-path>]`
9. If blocked or failed, checkpoint failure:
   `gjc ultragoal checkpoint --goal-id <id> --status failed --evidence "<blocker/evidence>"`
10. For legacy per-story cgjcleted-goal blockers, preserve the non-terminal blocker with:
   `gjc ultragoal checkpoint --goal-id <id> --status blocked --evidence "<cgjcleted legacy Codex goal blocks create_goal in this thread>" --codex-goal-json <get_goal-json-or-path>`
11. Resume failed goals with `gjc ultragoal cgjclete-goals --retry-failed`.

## Dynamic steering

Use `gjc ultragoal steer` when real findings or blockers prove the current story decgjcosition should change while the aggregate objective and constraints stay fixed. Steering is explicit-only and evidence-backed; broad natural-language requests are rejected instead of guessed.
...
- Do not edit the aggregate Codex objective, original brief constraints, quality gates, or cgjcletion status. The aggregate objective is a stable pointer to `.gjc/ultragoal/goals.json` and `.gjc/ultragoal/ledger.jsonl`, not an enumeration of initial goal ids.
- Do not hard-delete goals, auto-cgjclete work, weaken verification, or silently mutate `.gjc/ultragoal`.
...
- Blocked goals without replacements are skipped for scheduling but still block final cgjcletion until later explicit steering replaces or supersedes them.

UserPrgjctSubmit uses the same steering API only for structured directives such as `GJC_ULTRAGOAL_STEER: { ... }`, `gjc.ultragoal.steer: { ... }`, or `gjc ultragoal steer: { ... }`. Normal prose does not mutate state, and repeated prgjct-submit directives dedupe by prgjct signature or idempotency key.
...
```sh
gjc ultragoal checkpoint --goal-id <id> --status cgjclete --evidence "<team evidence mentioning .gjc/ultragoal and <id>>" --codex-goal-json <fresh-get_goal-json-or-path>
```
...
The final ultragoal story is not cgjclete until the active agent has run the final quality gate:
...
5. If review is non-clean, do **not** call `update_goal`. Record durable blocker work instead:
   ...
   This marks the current story `review_blocked`, appends a pending blocker-resolution story, keeps the Codex goal active, and lets `gjc ultragoal cgjclete-goals` start the blocker next. In legacy per-story mode, the blocker may need an available Codex goal context because the old per-story Codex goal remains active/incgjclete.

6. If review is clean, call `update_goal({status: "cgjclete"})`, call `get_goal`, and checkpoint with a structured final gate:

   ```sh
   gjc ultragoal checkpoint --goal-id <id> --status cgjclete --evidence "<tests/files/review evidence>" --codex-goal-json <fresh-cgjclete-get-goal-json-or-path> --quality-gate-json <quality-gate-json-or-path>
   ```
...
- After a cgjcleted aggregate ultragoal run, clear the Codex goal manually with `/goal clear` before starting another ultragoal run in the same session/thread.
- Never call `create_goal` when `get_goal` reports a different active goal.
- Never call `update_goal` unless the aggregate run or legacy per-story goal is actually cgjclete.
- In aggregate mode, intermediate story checkpoints require a matching `active` Codex snapshot; final story cgjcletion requires a matching `cgjclete` snapshot after `update_goal`.
- Cgjcletion checkpoints require read-only Codex snapshot reconciliation: pass fresh `get_goal` JSON/path with `--codex-goal-json`; shell commands and hooks must not mutate Codex goal state.
