...
```text
Config sources (.gjc/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.gjc/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.gjc/mcp.json` (project) or `~/.gjc/agent/mcp.json` (user) for explicit control.
