...
def test_chown_workspace_runs_chown_and_chmod_as_root_on_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 2001 is the slot-private GID matching the slot UID, not the shared gjc group.
    assert calls == [
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert tmpdir == tmp_path / ".gjc-tmp"
...
def test_provision_runtime_dirs_replaces_tmpdir_symlink_and_creates_xdg_tree(tmp_path: Path) -> None:
    ...
    assert target.is_dir()
    assert stat.S_IMODE(tmpdir.stat().st_mode) == 0o700
    for base in (tmp_path / ".gjc-xdg" / "data", tmp_path / ".gjc-xdg" / "state", tmp_path / ".gjc-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "gjc").is_dir()
    assert (tmp_path / ".gjc-xdg" / "cache" / "bun-install").is_dir()
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    bun_cache = ws.root / ".gjc-xdg" / "cache" / "bun-install"

    env = _prepare_slot_runtime_env(ws, 2001)

    assert env["TMPDIR"] == str(ws.root / ".gjc-tmp")
    assert env["XDG_CACHE_HOME"] == str(ws.root / ".gjc-xdg" / "cache")
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bun_cache)
    for base in (ws.root / ".gjc-xdg" / "data", ws.root / ".gjc-xdg" / "state", ws.root / ".gjc-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "gjc").is_dir()
...
def test_ensure_workspace_provisions_and_slot_owns_runtime_dirs(
    ...
    def record_chown(ws_root: Path, slot_uid: int | None) -> None:
        ...
        paths = [
            ws_root / ".gjc-tmp",
            ws_root / ".gjc-xdg" / "data",
            ws_root / ".gjc-xdg" / "data" / "gjc",
            ws_root / ".gjc-xdg" / "state",
            ws_root / ".gjc-xdg" / "state" / "gjc",
            ws_root / ".gjc-xdg" / "cache",
            ws_root / ".gjc-xdg" / "cache" / "gjc",
            ws_root / ".gjc-xdg" / "cache" / "bun-install",
        ]
    ...
    assert set(runtime_paths) == {
        ws.root / ".gjc-tmp",
        ws.root / ".gjc-xdg" / "data",
        ws.root / ".gjc-xdg" / "data" / "gjc",
        ws.root / ".gjc-xdg" / "state",
        ws.root / ".gjc-xdg" / "state" / "gjc",
        ws.root / ".gjc-xdg" / "cache",
        ws.root / ".gjc-xdg" / "cache" / "gjc",
        ws.root / ".gjc-xdg" / "cache" / "bun-install",
    }
