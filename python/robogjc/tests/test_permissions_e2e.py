...
def _require_linux_root_toolchain() -> None:
    if platform.system() != "Linux" or os.geteuid() != 0:
        pytest.skip("slot permission e2e tests require Linux root so subprocesses can drop to gjc-N UIDs")
...
def test_slot_workspace_runs_bun_biome_cargo_and_git_after_root_reentry(
    ...
    stale_bun_cache = first.root / ".gjc-xdg" / "cache" / "bun-install" / "root-owned-stale"
...
def _prepare_shared_natives_cache(slot_tmp_path: Path) -> NativesCache:
    """Provision `/data/cache/pi-natives` shape (root:gjc, setgid 2770)."""
...
def test_natives_cache_shares_artifacts_across_slot_workspaces(
    ...
    1. A capture from a slot-owned workspace lands in the shared cache with
       group `gjc` setgid inheritance so any other slot can read it.
    ...
    # Cache root is setgid `gjc`; new files inherit gid `gjc` so any slot
    # with `extra_groups=[gjc]` can read them.
    ...
    # Slot 2 must be able to read the populated artifacts (group gjc + 0660
