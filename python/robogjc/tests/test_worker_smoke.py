...
Runs only when ROBGJC_INTEGRATION=1 and `gjc` is available on PATH (or via
...
- a real `gjc --mode rpc` subprocess driven by `worker.run_task`.
...
pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBGJC_INTEGRATION=1 required to run the gjc-backed smoke test",
)
