...
Ownership: cache root is provisioned ``root:gjc 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``gjc``) can capture under setgid inheritance.
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `gjc` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `gjc` from the setgid 2770
                # cache root.
