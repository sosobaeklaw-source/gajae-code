...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".gjc/agent")):
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    """Build the env overlay passed to the gjc subprocess.

    `gjc_rpc` merges this dict on top of `os.environ`, so overlaying empty
...
def _has_prior_session(session_dir: Path) -> bool:
    """Return True iff `session_dir` already contains an gjc JSONL transcript.
...
def _run_rpc_blocking(
    ...
    *,
    ...
    with RpcClient(
        ...
        extra_groups=["gjc"] if inputs.slot_uid is not None else None,
    ) as client:
        # Arm cancellation: from this point the API can kill the gjc subprocess
        ...
        # it's idempotent.
        #
        # NOTE: gjc_rpc.RpcClient.stop() has a bug where it sets `_stopping=True`
        ...
        try:
            ...
            if hard_timeout_fired.is_set():
                raise TimeoutError("gjc task exceeded hard timeout")
            log.info(
                "rpc_done",
                extra={
                    "issue": bindings.issue_key,
