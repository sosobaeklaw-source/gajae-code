"""Host tools exposed to the agent through `gjc_rpc.host_tool`.
...
from gjc_rpc import HostTool, HostToolContext, RpcCommandError, host_tool
...
class AbortController:
    ...
    teardown of the gjc subprocess. The worker pre-populates `stop` with a
...
class ToolBindings:
    ...
    # Set by the worker before launching gjc. Carries the abort-task signal
...
def _build_abort_task(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        # races against the imminent gjc teardown.
